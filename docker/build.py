...
def build_image(image):
    subprocess.check_call(
        [
            "docker",
            "build",
            "--no-cache=%s" % ("true" if arguments.no_cache else "false"),
            "-t",
            "sandialabs/%s" % image,
            image,
        ]
...
def save_image(image):
    ...
    subprocess.check_call(
        ["docker", "save", "-o", saved_image, "sandialabs/%s" % image]
    )
...
parser.add_argument(
    "--no-cache", action="store_true", help="Don't use existing cached images."
)
