...
log.error("couch serverURL:%s" % serverURL)
while True:
  try:
...
single_node_server_names = ["_users", "_replicator", "_global_changes"]
for name in single_node_server_names:
    if name not in server:
        log.error("Saving database %s" % (name))
        server.create(name)
