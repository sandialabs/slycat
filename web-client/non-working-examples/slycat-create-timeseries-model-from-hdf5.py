...
parser.add_argument(
    "directory",
    help="Directory containing hdf5 timeseries data (one inputs.hdf5 and multiple timeseries-N.hdf5 files).",
)
parser.add_argument(
...
parser.add_argument(
    "--cluster-sample-type",
    default="uniform-paa",
    choices=["uniform-pla", "uniform-paa"],
...
parser.add_argument(
    "--model-description",
    default=None,
    help="New model description.  Defaults to a summary of the input parameters.",
)
parser.add_argument(
    "--model-name",
    default=None,
    help="New model name.  Defaults to the name of the input data directory.",
)
parser.add_argument(
    "--project-description",
    default="",
...
parser.add_argument(
    "--project-name",
    default="Timeseries Project",
    help="New or existing project name.  Default: %(default)s",
)
...
if arguments.model_description is None:
    ...
    arguments.model_description += (
        "Cluster sampling algorithm: %s.\n"
        % {"uniform-pla": "Uniform PLA", "uniform-paa": "Uniform PAA"}[
            arguments.cluster_sample_type
        ]
    )
    arguments.model_description += "Cluster method: %s.\n" % arguments.cluster_type
    arguments.model_description += (
        "Cluster distance metric: %s.\n" % arguments.cluster_metric
    )
...
pid = connection.find_or_create_project(
    arguments.project_name, arguments.project_description
)
...
try:
    ...
    for timeseries_index in range(_numTimeseries):
        with h5py.File(
            os.path.join(arguments.directory, "timeseries-%s.hdf5" % timeseries_index),
            "r",
        ) as file:
            timeseries_samples[timeseries_index] = len(
                slycat.hdf5.ArraySet(file)[0].get_data(0)[:]
            )
    ...
    if timeseries_samples.min() < _numSamples:
        ...
        slycat.web.client.log.info(
            "Reducing cluster sample count to minimum found in data: %s", _numSamples
        )
    ...
    connection.put_model_parameter(
        mid, "cluster-bin-type", arguments.cluster_sample_type
    )
    ...
    with h5py.File(os.path.join(arguments.directory, "inputs.hdf5"), "r") as file:
        ...
        for attribute in range(len(attributes)):
            ...
            connection.put_model_arrayset_data(
                mid, "inputs", "0/%s/..." % attribute, [data]
            )
    ...
    for timeseries_index in range(timeseries_count):
        with h5py.File(
            ...
            attributes = slycat.hdf5.ArraySet(file)[0].attributes[
                1:
            ]  # Skip the timestamps
            timeseries_samples[timeseries_index] = len(
                slycat.hdf5.ArraySet(file)[0].get_data(0)[:]
    ...
    connection.post_model_files(
        mid,
        ["clusters"],
        [json.dumps(sorted(clusters.keys()))],
    ...
    def get_time_range(directory, timeseries_index):
        ...
        with h5py.File(
            os.path.join(directory, "timeseries-%s.hdf5" % timeseries_index), "r+"
        ) as file:  # We have to open the file with writing enabled in case the statistics cache gets updated.
    ...
    time_ranges = pool[:].map_sync(
        get_time_range,
        list(itertools.repeat(arguments.directory, timeseries_count)),
    ...
    for index, (name, storage) in enumerate(sorted(clusters.items())):
        ...
        connection.update_model(
            mid, message="Resampling data for %s" % name, progress=progress_begin
        )
        ...
        if arguments.cluster_sample_type == "uniform-pla":
            ...
            def uniform_pla(
                ...
                with h5py.File(
                    os.path.join(directory, "timeseries-%s.hdf5" % timeseries_index),
                    "r",
                ) as file:
                    original_times = slycat.hdf5.ArraySet(file)[0].get_data(0)[:]
                    original_values = slycat.hdf5.ArraySet(file)[0].get_data(
                        attribute_index + 1
                    )[:]
            ...
            def uniform_paa(
                ...
                bin_sums = numpy.bincount(
                    bin_indices, original_values, minlength=bin_count + 1
                )[1:]
                lonely_bins = bin_counts < 2
                bin_counts[lonely_bins] = 1
                bin_sums[lonely_bins] = numpy.interp(
                    bin_times, original_times, original_values
                )[lonely_bins]
        ...
        for i in range(0, observation_count):
            ...
            slycat.web.client.log.info(
                "Computing distance matrix for %s, %s of %s"
                % (name, i + 1, observation_count)
            )
            for j in range(i + 1, observation_count):
                distance = numpy.sqrt(
                    numpy.sum(
                        numpy.power(
                            waveforms[j]["values"] - waveforms[i]["values"], 2.0
                        )
                    )
                )
        ...
        linkage = scipy.cluster.hierarchy.linkage(
            distance,
            method=str(arguments.cluster_type),
        ...
        for i in range(len(linkage)):
            ...
            cluster_membership.append(
                cluster_membership[cluster1].union(cluster_membership[cluster2])
            )
            ...
            for cluster1_member in cluster_membership[cluster1]:
                for cluster2_member in cluster_membership[cluster2]:
                    summed_distances[cluster1_member] += distance_matrix[
                        cluster1_member
                    ][cluster2_member]

            for cluster2_member in cluster_membership[int(cluster2)]:
                for cluster1_member in cluster_membership[cluster1]:
                    summed_distances[cluster2_member] += distance_matrix[
                        cluster2_member
                    ][cluster1_member]
            ...
            for member in cluster_membership[cluster_id]:
                if (
                    min_summed_distance is None
                    or summed_distances[member] < min_summed_distance
                ):
                    min_summed_distance = summed_distances[member]
                    exemplar_id = member

                if (
                    max_summed_distance is None
                    or summed_distances[member] > min_summed_distance
                ):
                ...
                json.dumps(
                    {
                        ...
                        "exemplars": exemplars,
                        "input-indices": [
                            waveform["input-index"] for waveform in waveforms
                        ],
                    }
                )
            ],
        ...
        for index, waveform in enumerate(waveforms):
            ...
            attributes = [
                ...
                dict(name="value", type="float64"),
            ]
            slycat.web.client.log.debug(
                "Creating array %s %s" % (attributes, dimensions)
            )
            connection.put_model_arrayset_array(
                mid, arrayset_name, index, dimensions, attributes
            )
            slycat.web.client.log.debug(
                "Uploading %s times, %s values"
                % (waveform["times"].shape, waveform["values"].shape)
            )
            connection.put_model_arrayset_data(
                mid,
                arrayset_name,
...
slycat.web.client.log.info(
    "Your new model is located at %s/models/%s" % (arguments.host, mid)
)
