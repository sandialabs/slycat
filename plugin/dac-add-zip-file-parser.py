...
def parse_add_zip(database, model, input, files, aids, **kwargs):
    ...
    cherrypy.log.error("DAC PTS Add Zip parser started.")
    ...
    thread = threading.Thread(target=parse_pts_thread, args=(database, model, zip_ref,
                                                             csv_files, meta_files, csv_no_ext,
...
def register_slycat_plugin(context):
    context.register_parser("dac-add-zip-file-parser", "PTS CSV/META .zip file", ["dac-add-zip-file"], parse_add_zip)
