define("slycat-parameter-image-model-login", ["slycat-server-root"], function(server_root) {
  function Login() {
    var self = this;
    this.session_cache = {};
    this.grid_ref = '#scatterplot-pane'; //TODO don't hardcode the DOM
    this.image_uri = document.createElement("a");

    // Setup the login dialog ...
    self.image_login = $("#login-modal");
  ...
  Login.prototype.show_prompt = function(images, callback, this_arg) {
    ...
    $("#login-button").on('click', function() {
      $.ajax({
        ...
        data : JSON.stringify({hostname: parser.hostname, username: $("#remote-username").val(), password: $("#remote-password").val()}),
        processData : false,
