...
$.widget("mp.controls", {
  ...
  _create: function () {
    ...
    this.color_items = $(
      '<div id="y-axis-switcher" class="dropdown-menu" aria-labelledby="color-dropdown">',
    ).appendTo(self.color_control);
    ...
    this.pin_selected_button = $(
      ...
      .click(function () {
        ...
        var newPins = _.difference(
          self.options.highlighted_simulations,
          self.options.pinned_simulations,
        );
        ...
        // Otherwise, create a new array of all highlighted_simulations and pinned simulations and trigger with that new array. Don't update local pinned_simulations.
          ...
          self.element.trigger("pinned_simulations_changed", [
            _.union(self.options.pinned_simulations, newPins),
          ]);
        }
      })
  ...
  _write_data_table: function (selectionList) {
    ...
    $.ajax({
      type: "POST",
      url:
        api_root + "models/" + self.options.model._id + "/arraysets/" + self.options.aid + "/data",
      data: JSON.stringify({ hyperchunks: "0/.../..." }),
      contentType: "application/json",
      success: function (result) {
        self._write_csv(
          self._convert_to_csv(result, selectionList),
          self.options.model.name + "_data_table.csv",
        );
      },
  ...
  _set_selected_color: function () {
    ...
    self.color_items.find("a").removeClass("active");
    self.color_items
      .find('a[data-colorvariable="' + self.options.color_variable + '"]')
      .addClass("active");
  },
  ...
  _set_pin_item_state: function () {
    ...
    self.pin_selected_button.prop(
      "disabled",
      _.isEmpty(
        _.difference(self.options.highlighted_simulations, self.options.pinned_simulations),
      ),
    );
  },
  ...
  _respond_pinned_simulations_changed: function () {
    ...
    $("button", this.playback_controls).prop(
      "disabled",
      !(self.options.video_sync || current_frame_video),
    );
    ...
    if (
      (self.options.video_sync && any_video_playing) ||
      (!self.options.video_sync && current_frame_video_playing)
    ) {
      self.pause_button.show();
