  ...
  _create: function()
  {
    ...
    function cell_formatter(row, cell, value, columnDef, dataContext)
    {
      if(columnDef.color_scale)
        return "<div class='highlightWrapper" + (value==null ? " null" : "") + ( d3.hcl(columnDef.color_scale(value)).l > 50 ? " light" : " dark") + "' style='background:" + columnDef.color_scale(value) + "'>" + value_formatter(value) + "</div>";
      else if(value==null)
    ...
    function make_column(column_index, header_class, cell_class)
    {
      return {
        ...
        header :
        {
          ...
          [
            {
              cssClass : self.options.sort_variable == column_index ? (self.options.sort_order == "ascending" ? "icon-sort-ascending" : "icon-sort-descending") : "icon-sort-off",
              tooltip : self.options.sort_variable == column_index ? (self.options.sort_order == "ascending" ? "Sort descending" : "Sort ascending") : "Sort ascending",
              command : self.options.sort_variable == column_index ? (self.options.sort_order == "ascending" ? "sort-descending" : "sort-ascending") : "sort-ascending"
            }
          ]
        }
      };
    ...
    {
      if( (args.column.field != self.options.color_variable) && (self.options.metadata["column-types"][args.column.id] != "string") )
      {
