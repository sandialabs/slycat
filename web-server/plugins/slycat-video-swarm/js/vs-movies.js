...
$.widget("mp.movies", {
  ...
  _handle_color_change: function () {
    ...
    $("#mp-movies .videoContainer").each(function (index, element) {
      ...
      $(element).css({
        "border-color": self.options.color_scale(
          self.options.table_data[0].data[self.options.color_var_index][index],
        ),
      });
    });
  },
    ...
    for (var i = 0; i < simulation_indexes.length; i++) {
      ...
      if (self.options.current_video == simulation_indexes[i]) {
        self._set_current_video(
          self.options.pinned_simulations[0] == undefined
            ? null
            : self.options.pinned_simulations[0],
        );
      }
      $(
        ".videoContainer[data-index='" + simulation_indexes[i] + "']",
  ...
  _open_movies: function (simulation_indexes) {
    ...
    if (movie_index in self.options.movie_cache) {
      ...
      var download_button = $(
        '<a class="download-button frame-button fa fa-download" title="Download media file"></a>',
      )
        .attr("href", movie_url)
              ...
              xhr.onload = function (e) {
                ...
                if (this.status == 400) {
                  ...
                  if (message && hint) {
                    ...
                    window.alert(
                      "Error loading image " + this.movie_index + ": " + this.statusText,
                    );
                  }
  ...
  frame_back: function () {
    ...
    if (self.options.video_sync) {
      ...
      if (firstVideo != undefined) {
        self.options.video_sync_time = Math.max(
  ...
  frame_forward: function () {
    ...
    if (self.options.video_sync) {
      ...
      var firstVideo = videos.get(0);
      if (firstVideo != undefined) {
        self.options.video_sync_time = Math.min(
          firstVideo.currentTime + self.options.frameLength,
