...
$.widget("slycat.colorswitcher", {
  ...
  _create: function () {
    ...
    this.button = $(
      '<button class="btn dropdown-toggle btn-sm btn-slycat-controls" type="button" id="colors-dropdown" data-bs-toggle="dropdown" aria-expanded="false" title="Scatterplot Color Theme"> \
          Colors \
        </button>',
    ).appendTo(this.element);
    this.list = $('<div class="dropdown-menu" aria-labelledby="colors-dropdown">').appendTo(
      this.element,
    );
  ...
  get_color_scale: function (name, min, max) {
    ...
    if (name === undefined) name = this.options.colormap;
    if (min === undefined) min = -500.0;
    if (max === undefined) max = -144.0;
    var domain = [];
