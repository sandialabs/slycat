...
def register_slycat_plugin(context):
    ...
    def list_uris(database, model, verb, type, command, **kwargs):
        ...
        try:
            ...
            cherrypy.log.error(
                "slycat-remap-wizard.py list_uris",
                "cherrypy.HTTPError 400 missing / invalid columns parameter.",
            )
            ...
            tmp_uris = next(
                iter(
                    slycat.web.server.get_model_arrayset_data(
                        database, model, "data-table", "0/%s/..." % column
                    )
                )
            )
    ...
    def search_and_replace(database, model, verb, type, command, **kwargs):
        ...
        try:
            ...
            cherrypy.log.error(
                "slycat-remap-wizard.py search_and_replace",
                "cherrypy.HTTPError 400 missing / invalid columns parameter.",
            )
            raise cherrypy.HTTPError("400 Missing / invalid columns parameter.")
        ...
        try:
            ...
            cherrypy.log.error(
                "slycat-remap-wizard.py search_and_replace",
                "cherrypy.HTTPError 400 missing / invalid replace parameter.",
            )
        ...
        try:
            ...
            cherrypy.log.error(
                "slycat-remap-wizard.py search_and_replace",
                "cherrypy.HTTPError 400 missing / invalid search parameter.",
            )
        ...
        for attribute in columns:
            before = next(
                iter(
                    slycat.web.server.get_model_arrayset_data(
                        database, model, "data-table", "0/%s/..." % attribute
                    )
                )
            )
            ...
            slycat.web.server.put_model_arrayset_data(
                database, model, "data-table", "0/%s/..." % attribute, [after]
            )

        return json.dumps({"ok": True})

    context.register_model_command(
        "POST", "remap-wizard", "search-and-replace", search_and_replace
    )
