...
def register_slycat_plugin(context):
  ...
  def compute(mid):
    try:
      ...
      if len(input_columns) < 1:
        cherrypy.log.error("slycat-cca-old.py compute", "CCA model requires at least one input column.")
        raise Exception("CCA model requires at least one input column.")
      if len(output_columns) < 1:
        cherrypy.log.error("slycat-cca-old.py compute", "CCA model requires at least one output column.")
  ...
  context.register_model("cca-old", finish)

  # Register custom wizards for creating CCA models.
  context.register_wizard("new-cca-old", "New CCA Old Model", require={"action":"create", "context":"project"})
  context.register_wizard("rerun-cca-old", "Modified CCA Old Model", require={"action":"create", "context":"model", "model-type":["cca-old"]})
