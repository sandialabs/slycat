...
Movie.prototype.build_open_button = function(container, plot) {
  ...
  self.open_control = container.append('g')
    ...
    .on('click', function() {
      ...
      if(!self.agent_id){
        self.agent_id = login.get_agent(plot.images.filter(function(x){return x;})[0]);
      }
      self.agent_id ? self.build_server_movie.call(self, plot) : login.show_prompt(plot.images.filter(function(uri){return uri;}).map(function(uri){return {uri: uri};}), function(){self.build_server_movie(plot)}, self);
      plot.movie = self;
    })
...
Movie.prototype.build_server_movie = function(plot) {
  ...
  self.agent_id = login.get_agent(plot.images.filter(function(url){return url && url.length > 0})[0]);
  $('.open-movie').hide();

  if(!self.movie[image_index]){
    model.get_image_column(plot.images_index, function(images) {
      $.ajax({
        type: "POST",
