...
def register_slycat_plugin(context):
    ...
    def finish(model_id):
        ...
        slycat.web.server.update_model(
            ...
            progress=100,
            message="timeseries model finished uploading all data",
        )
    ...
    def get_remote_file_server(
        hostname, model, filename, total_file_delta_time=[], calling_client=None
    ):
        """
    ...
    def get_sid(hostname, model):
        ...
        try:
            ...
            sessions = [
                session
                for session in database.scan("slycat/sessions")
                if session["creator"] == model["creator"]
            ]
            if len(sessions) > 1:
            ...
            cherrypy.log.error(
                "Timeseries model compute exception type: %s" % sys.exc_info()[0]
            )
            cherrypy.log.error(
                "Timeseries model compute exception value: %s" % sys.exc_info()[1]
            )
            cherrypy.log.error(
                "Timeseries model compute exception traceback: %s" % sys.exc_info()[2]
            )
    ...
    def helpGetFile(
        ...
        if use_tar:
            ...
            return get_remote_file_server(
                hostname, model, filename, total_file_delta_time, calling_client
            )
    ...
    def compute(model_id, stop_event, calling_client):
        ...
        try:
            ...
            database = slycat.web.server.database.couchdb.connect()
            model = database.get("model", model_id)
            model["model_compute_time"] = datetime.datetime.now(
            ...
            slycat.web.server.update_model(
                ...
                model,
                state="waiting",
                message="starting data pull Timeseries",
            )
            model = database.get("model", model_id)
            uid = slycat.web.server.get_model_parameter(database, model, "pickle_uid")
            workdir_raw = slycat.web.server.get_model_parameter(
                database, model, "working_directory"
            )
            workdir = workdir_raw + "pickle"
            hostname = slycat.web.server.get_model_parameter(
                database, model, "hostname"
            )
            username = slycat.web.server.get_model_parameter(
                database, model, "username"
            )
            ...
            # load inputs
            slycat.web.server.update_model(
                database, model, progress=50, message="loading inputs"
            )
            use_tar = True
            ...
            input_tar = None
            try:
                myfiles_tar_gz = get_remote_file_server(
                    hostname,
                    model,
            ...
            inputs = helpGetFile(
                "%sarrayset_inputs.pickle" % (pickle_path),
                use_tar,
                hostname,
                model,
                total_file_delta_time,
            ...
            # load attributes
            slycat.web.server.update_model(
                database, model, progress=55, message="loading attributes"
            )
            attributes = inputs["attributes"]
            slycat.web.server.put_model_array(
                database, model, inputs["aid"], 0, attributes, inputs["dimensions"]
            )
            # load attribute data
            data = helpGetFile(
                "%sinputs_attributes_data.pickle" % (pickle_path),
                use_tar,
                hostname,
                model,
                total_file_delta_time,
            ...
            # load clusters data
            slycat.web.server.update_model(
                database, model, progress=60, message="loading cluster data"
            )
            clusters = helpGetFile(
                "%sfile_clusters.json" % (pickle_path),
                use_tar,
                hostname,
                model,
                total_file_delta_time,
            ...
            slycat.web.server.update_model(
                ...
                model,
                progress=65,
                message="Pulling timeseries computed data for %s cluster files"
                % len(clusters_file),
            )
            progress = 65
            ...
            for file_name in clusters_file:
                ...
                slycat.web.server.update_model(
                    ...
                    model,
                    progress=progress,
                    message="loading %s cluster file" % file_name,
                )
                file_cluster_data = helpGetFile(
                    "%sfile_cluster_%s.json" % (pickle_path, file_name),
                    use_tar,
                    hostname,
                    model,
                    total_file_delta_time,
                ...
                slycat.web.server.post_model_file(
                    ...
                    file_cluster_attr["aid"],
                    file_cluster_attr["parser"],
                    client=calling_client,
                )
                database = slycat.web.server.database.couchdb.connect()
                model = database.get("model", model["_id"])
                slycat.web.server.put_model_arrayset(
                    database, model, "preview-%s" % file_name
                )

                waveform_dimensions_data = helpGetFile(
                    "%swaveform_%s_dimensions.pickle" % (pickle_path, file_name),
                    use_tar,
                    hostname,
                    model,
                    total_file_delta_time,
                ...
                waveform_attributes_data = helpGetFile(
                    "%swaveform_%s_attributes.pickle" % (pickle_path, file_name),
                    use_tar,
                    hostname,
                    model,
                    total_file_delta_time,
                ...
                waveform_times_data = helpGetFile(
                    "%swaveform_%s_times.pickle" % (pickle_path, file_name),
                    use_tar,
                    hostname,
                    model,
                    total_file_delta_time,
                ...
                waveform_values_data = helpGetFile(
                    "%swaveform_%s_values.pickle" % (pickle_path, file_name),
                    use_tar,
                    hostname,
                    model,
                    total_file_delta_time,
                ...
                for index in range(int(timeseries_count)):
                    try:
                        ...
                        slycat.web.server.put_model_arrayset_data(
                            ...
                            "preview-%s" % file_name,
                            "%s/0/...;%s/1/..." % (index, index),
                            [waveform_times_array[index], waveform_values_array[index]],
                        )
                    except:
            ...
            model = database.get("model", model_id)
            slycat.web.server.update_model(
                database, model, message="finished loading all data"
            )
            ...
            cherrypy.log.error(traceback.format_exc())
            cherrypy.log.error(
                "Timeseries cperror model compute exception type: %s"
                % sys.exc_info()[0]
            )
            cherrypy.log.error(
                "Timeseries model compute exception value: %s" % sys.exc_info()[1]
        ...
        file = get_remote_file_server(
            hostname,
            model,
            "/home/%s/slurm-%s.out" % (username, model["artifact:jid"]),
            total_file_delta_time,
            calling_client,
        )
    ...
    def get_job_status(hostname, jid):
        ...
        try:
            ...
            cherrypy.log.error(
                "Something went wrong while checking on job %s status %s check ssh session"
                % (jid, str(e))
            )
    ...
    def update_remote_job(mid, jid, hostname, calling_client):
        ...
        cherrypy.log.error(
            "[Timeseries] checkjob %s returned with status %s" % (jid, state)
        )

        if state in ["RUNNING", "PENDING"]:
            if state == "RUNNING":
                slycat.web.server.update_model(
                    database, model, progress=10, message="Job is in pending state"
                )
            else:
                slycat.web.server.update_model(
                    database, model, progress=5, message="Job is in pending state"
                )
            slycat.web.server.put_model_parameter(database, model, "computing", False)
            if "job_running_time" not in model and state == "RUNNING":
                model = database.get("model", model["_id"])
                model["job_running_time"] = datetime.datetime.now(
                    datetime.timezone.utc
                ).isoformat()
                with slycat.web.server.get_model_lock(model["_id"]):
                    database.save(model)
        ...
        if state == "COMPLETED":
            slycat.web.server.update_model(
                database, model, progress=50, message="Job is in Completed state"
            )
            ...
            if "job_completed_time" not in model:
                model = database.get("model", model["_id"])
                model["job_completed_time"] = datetime.datetime.now(
                    datetime.timezone.utc
                ).isoformat()
                with slycat.web.server.get_model_lock(model["_id"]):
                    database.save(model)
            ...
            thread = threading.Thread(
                target=compute, args=(model["_id"], stop_event, calling_client)
            )
        ...
        if state == ["FAILED", "UNKNOWN", "NOTQUEUED"]:
            cherrypy.log.error(
                "Something went wrong with job %s job state:" % (jid, state)
            )
            slycat.web.server.update_model(
                database,
                model,
                message="Job %s had returned a bad or unknown state from the hpc system"
                % jid,
            )
            slycat.web.server.put_model_parameter(database, model, "computing", False)
    ...
    def update_model_info(database, model, verb, type, command, **kwargs):
        ...
        """
        slycat.web.server.update_model(
            database, model, progress=1, message="Job has been sent to slurm"
        )
        model_params = {
            ...
            "job_submit_time": datetime.datetime.now(datetime.timezone.utc).isoformat(),
        }
        for key, value in model_params.items():
            slycat.web.server.put_model_parameter(
                database, model, key, value, input=False
            )
    ...
    def pull_data(database, model, verb, type, command, **kwargs):
        ...
        try:
            cherrypy.log.error(
                "computing model value:"
                + str(
                    slycat.web.server.get_model_parameter(database, model, "computing")
                )
            )
            if model["_id"] in thread_pool:
                if thread_pool[model["_id"]].is_alive():
                    cherrypy.log.error(
                        "computing thread is alive for model %s" % str(model["_id"])
                    )
                else:
                    cherrypy.log.error(
                        "computing thread is dead for model %s setting compute to false"
                        % str(model["_id"])
                    )
                    del thread_pool[model["_id"]]
                    slycat.web.server.put_model_parameter(
                        database, model, "computing", False
                    )
            else:
                slycat.web.server.put_model_parameter(
                    database, model, "computing", False
    ...
    context.register_model_command(
        "POST", "timeseries", "update-model-info", update_model_info
    )
