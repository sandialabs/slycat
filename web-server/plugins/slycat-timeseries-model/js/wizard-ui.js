...
function constructor(params) {
  ...
  var updateUserConfig = function () {
    ...
    component.user_config["timeseries-wizard"]["inputs-file-delimiter"] =
      component.inputs_file_delimiter();
  ...
  var conditionallySetHdf5Path = function () {
    if (component.workdir() != "") {
      var hdf5_directory_browser = ko.dataFor(
        $(".slycat-remote-browser.timeseries-hdf5-directory")[0]
      );
      ...
      client.get_user_config({
        ...
        success: function (response) {
          ...
          if (response.config["timeseries-wizard"]) {
            ...
            response.config["timeseries-wizard"]["timeseries-name"]
              ? component.timeseries_name(response.config["timeseries-wizard"]["timeseries-name"])
              : null;
            response.config["timeseries-wizard"]["id-column"]
              ? component.id_column(response.config["timeseries-wizard"]["id-column"])
              : null;
            response.config["timeseries-wizard"]["inputs-file-delimiter"]
              ? component.inputs_file_delimiter(
                  response.config["timeseries-wizard"]["inputs-file-delimiter"]
                )
              : null;
          ...
          if (response.config.slurm) {
            ...
            response.config.slurm.partition
              ? component.partition(response.config.slurm.partition)
              : null;
            ...
            response.config.slurm["ntasks-per-node"]
              ? vm.ntasks_per_node(response.config.slurm["ntasks-per-node"])
              : null;
            ...
            if (!time_recommended) {
              ...
              response.config.slurm["time-hours"]
                ? vm.time_hours(response.config.slurm["time-hours"])
                : null;
              response.config.slurm["time-minutes"]
                ? vm.time_minutes(response.config.slurm["time-minutes"])
                : null;
              response.config.slurm["time-seconds"]
                ? vm.time_seconds(response.config.slurm["time-seconds"])
                : null;
            }
          }
            ...
            success: function (response) {
              ...
              if (response.config.slurm) {
                ...
                response.config.slurm.partition
                  ? component.partition(response.config.slurm.partition)
                  : null;
                response.config.slurm.workdir
                  ? component.workdir(response.config.slurm.workdir)
                  : null;
                conditionallySetHdf5Path();
      ...
      component.timeseries_names(
        client.get_time_series_names({
          ...
          error: function (request, status, reason_phrase) {
            ...
            dialog.dialog({
              message: "Please select a CSV file with a valid timeseries column.",
            })();
          },
        })
      );
  ...
  component.select_hdf5_directory = function () {
    dialog.confirm({
      ...
      message:
        "Please confirm this is the directory containing your HDF5 files: <br />" +
        component.remote_hdf5.path(),
      ok: function () {
