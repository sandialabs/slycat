...
$.widget("timeseries.waveformplot", {
  ...
  _create: function () {
    ...
    this.canvas_datum = d3
      .select(self.element.parent().get(0))
    ...
    function hover(event) {
      ...
      var rgbaPick =
        "rgba(" + dataPick[0] + "," + dataPick[1] + "," + dataPick[2] + "," + dataPick[3] + ")";
  ...
  _calculate_paths: function () {
    var self = this;
    var x_min = d3.min(this.waveforms, function (waveform) {
      return d3.min(waveform["time"]);
    });
    var x_max = d3.max(this.waveforms, function (waveform) {
      return d3.max(waveform["time"]);
    });
    var y_min = d3.min(this.waveforms, function (waveform) {
      return d3.min(waveform["value"]);
    });
    var y_max = d3.max(this.waveforms, function (waveform) {
      return d3.max(waveform["value"]);
    });
  ...
  _set_visible: function () {
    ...
    this.x_axis_layer
      .attr(
        ...
        "translate(" +
          (this.padding_left - 1) +
          "," +
          (this.padding_top + this.diagram_height + 1) +
          ")"
      )
      .call(this.x_axis);
    ...
    this.y_axis_layer
      .attr(
        "transform",
        "translate(" + (this.padding_left - 1) + "," + (this.padding_top + 1) + ")"
      )
      .call(this.y_axis);

    d3.selectAll("canvas").attr("width", this.diagram_width).attr("height", this.diagram_height);
    ...
    var color_scale_and_color_array =
      self.options.color_scale != null && self.options.color_array != null;
    ...
    function processWaveform(waveform) {
      var coloredLine =
        color_scale_and_color_array && self.options.color_array[waveform["input-index"]] !== null;
      if (coloredLine) {
        var strokeStyle = self.options.color_scale(
          self.options.color_array[waveform["input-index"]]
        );
      } else {
        var strokeStyle = slycat_color_maps.get_null_color(
          self.options.get_state().controls.colormap
        );
  ...
  _selection: function (waveforms) {
    ...
    // Clear the canvas
    self.canvas_selection_ctx.clearRect(
      0,
      0,
      self.canvas_selection.width,
      self.canvas_selection.height
    );
