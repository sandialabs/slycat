...
$.widget("timeseries.table", {
    ...
    function cell_formatter(row, cell, value, columnDef, dataContext) {
      if (columnDef.colorscale) {
        return (
          ...
          (value == null ? " null" : "") +
          (d3.hcl(columnDef.colorscale(value)).l > 50 ? " light" : " dark") +
          "' style='background:" +
          columnDef.colorscale(value) +
          "'>" +
          ...
          buttons: [
            {
              cssClass:
                self.options["sort-variable"] == column_index
                  ? self.options["sort-order"] == "ascending"
                    ? "icon-sort-ascending"
    ...
    if (key == "row-selection") {
      ...
      function initialize_grid(indices) {
        ...
        if (indices !== undefined) {
          ...
          if (sorted_rows.length > 1) sorted_table_filter = Array.apply([], sorted_rows);
          else sorted_table_filter = [sorted_rows[0]];
        ...
        if (self.grid) {
          ...
          self.grid.onSelectedRowsChanged.subscribe(function (e, selection) {
            ...
            if (self.trigger_row_selection) {
              ...
              for (var i = 0; i < selection.rows.length; i++) {
                waveform_indexes.push(
                  self.grid.getDataItem(selection.rows[i])[
          ...
          self.grid.onHeaderClick.subscribe(function (e, args) {
            if (
              ...
              // self._color_variables(self.options["variable-selection"]);
              self.element.trigger("variable-selection-changed", {
                variable: self.options["variable-selection"],
                colorscale: args.column.colorscale,
              });
            }
          });
      ...
      if (
        self.options["sort-variable"] !== null &&
        self.options["sort-order"] !== null &&
        self.options.table_filter.length > 0
      ) {
        ...
        if (self.options["sort-variable"] == self.options.metadata["column-count"] - 1) {
          ...
          request.open(
            "GET",
            self.options.api_root +
              "models/" +
              self.options.mid +
              "/arraysets/" +
              self.options.aid +
              "/data?hyperchunks=0/rank(a" +
              self.options["sort-variable"] +
              ',"asc")/...&byteorder=' +
  ...
  _data_provider: function (parameters) {
    ...
    else self.sorted_table_filter = parameters.sorted_table_filter;
    if (parameters.retrieve_table_filter == undefined)
      self.retrieve_table_filter = parameters.table_filter;
    else self.retrieve_table_filter = parameters.retrieve_table_filter;
    ...
    self.set_sort = function (column, order, callback) {
      if (column == self.sort_column && order == self.sort_order) {
        ...
        self.get_indices("sorted", self.table_filter, function (sorted_rows) {
          if (sorted_rows.length > 1) self.sorted_table_filter = Array.apply([], sorted_rows);
          else self.sorted_table_filter = [sorted_rows[0]];
    ...
    self.getSimulationRowIndexes = function (simulation_indexes) {
      ...
      for (var i = 0; i < simulation_indexes.length; i++) {
        ...
        if (table_filter_index > -1) {
          ...
          retrieve_table_filter_index = self.retrieve_table_filter.indexOf(
            sorted_table_filter_element,
          );
