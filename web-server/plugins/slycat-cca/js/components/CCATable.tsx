...
class CCATable extends React.Component {
  constructor(props) {
    ...
    this.columns = [];
    this.columns.push(
      this.make_column(this.props.metadata["column-count"] - 1, "headerSimId", "rowSimId"),
    );
    ...
    return {
      ...
      header: {
        buttons: [
          {
            cssClass:
              this.props.sort_variable == column_index
                ? this.props.sort_order == "ascending"
                  ? "icon-sort-ascending"
  ...
  cell_formatter = (row, cell, value, columnDef, dataContext) => {
    if (columnDef.colormap)
      return (
        ...
        (value == null ? " null" : "") +
        (d3.hcl(columnDef.colormap(value)).l > 50 ? " light" : " dark") +
        "' style='background:" +
        columnDef.colormap(value) +
        "'>" +
  ...
  componentDidMount() {
    ...
    this.grid = new SlickGrid(this.cca_table.current, this.data, this.columns, {
    ...
    let header_buttons = new SlickHeaderButtons();
