...
def register_slycat_plugin(context):
    ...
    def compute(mid):
        try:
            ...
            input_columns = slycat.web.server.get_model_parameter(
                database, model, "input-columns"
            )
            output_columns = slycat.web.server.get_model_parameter(
                database, model, "output-columns"
            )
            scale_inputs = slycat.web.server.get_model_parameter(
                database, model, "scale-inputs"
            )

            # double check the number of inputs and outputs
            if len(input_columns) < 1:
                cherrypy.log.error(
                    "slycat-cca.py compute",
                    "CCA model requires at least one input column.",
                )
                raise Exception("CCA model requires at least one input column.")
            if len(output_columns) < 1:
                cherrypy.log.error(
                    "slycat-cca.py compute",
                    "CCA model requires at least one output column.",
                )
            ...
            X = numpy.column_stack(
                list(
                    slycat.web.server.get_model_arrayset_data(
                        database, model, "data-table", "0/%s/..." % input_attributes
                    )
                )
            )
            ...
            Y = numpy.column_stack(
                list(
                    slycat.web.server.get_model_arrayset_data(
                        database, model, "data-table", "0/%s/..." % output_attributes
                    )
                )
            )
            ...
            x, y, x_loadings, y_loadings, r, wilks = cca(
                X, Y, scale_inputs=scale_inputs
            )
            ...
            slycat.web.server.put_model_array(
                ...
                "canonical-indices",
                0,
                [dict(name="index", type="int32")],
                [dict(name="sample", end=sample_count)],
            )
            slycat.web.server.put_model_arrayset_data(
                database, model, "canonical-indices", "0/0/...", [indices]
            )
            ...
            slycat.web.server.put_model_array(
                ...
                0,
                [
                    dict(name="input", type="float64"),
                    dict(name="output", type="float64"),
                ],
                [
                    dict(name="component", end=component_count),
                    dict(name="sample", end=sample_count),
                ],
            )
            slycat.web.server.put_model_arrayset_data(
                database, model, "canonical-variables", "0/0/...;0/1/...", [x.T, y.T]
            )
            ...
            slycat.web.server.put_model_arrayset(
                database, model, "input-structure-correlation"
            )
            slycat.web.server.put_model_array(
                database,
                model,
                "input-structure-correlation",
                0,
                [dict(name="correlation", type="float64")],
                [
                    dict(name="component", end=component_count),
                    dict(name="input", end=len(input_columns)),
                ],
            )
            slycat.web.server.put_model_arrayset_data(
                database,
            ...
            slycat.web.server.put_model_arrayset(
                database, model, "output-structure-correlation"
            )
            slycat.web.server.put_model_array(
                database,
                model,
                "output-structure-correlation",
                0,
                [dict(name="correlation", type="float64")],
                [
                    dict(name="component", end=component_count),
                    dict(name="output", end=len(output_columns)),
                ],
            )
            slycat.web.server.put_model_arrayset_data(
                database,
            ...
            slycat.web.server.put_model_array(
                ...
                "cca-statistics",
                0,
                [dict(name="r2", type="float64"), dict(name="p", type="float64")],
                [dict(name="component", end=component_count)],
            )
            slycat.web.server.put_model_arrayset_data(
                database, model, "cca-statistics", "0/0/...;0/1/...", [r, wilks]
            )
    ...
    def finish(database, model):
        ...
        thread = threading.Thread(
            name="Compute CCA Model", target=compute, kwargs={"mid": model["_id"]}
        )
        thread.start()
    ...
    def cca(X, Y, scale_inputs=True, force_positive=None, significant_digits=None):
        ...
        if X.shape[0] != Y.shape[0]:
            cherrypy.log.error(
                "cca.py", "X and Y must contain the same number of rows."
            )
            raise ValueError("X and Y must contain the same number of rows.")
        if X.shape[0] < X.shape[1] or X.shape[0] < Y.shape[1]:
            cherrypy.log.error(
                "cca.py",
                "Number of rows must be >= the number of column in X, and >= the number of columns in Y.",
            )
            raise ValueError(
                "Number of rows must be >= the number of columns in X, and >= the number of columns in Y."
            )
        if X.shape[1] < 1 or Y.shape[1] < 1:
            cherrypy.log.error(
                "cca.py", "X and Y must each contain at least one column."
            )
        ...
        if force_positive is not None and (
            force_positive < 0 or force_positive >= Y.shape[1]
        ):
            cherrypy.log.error(
                "cca.py",
                "force_positive must be in the range [0, number of Y columns).",
            )
            raise ValueError(
                "force_positive must be in the range [0, number of Y columns)."
            )
        if not isinstance(significant_digits, (type(None), numbers.Integral)):
            cherrypy.log.error(
                "cca.py", "significant_digits must be an integer or None."
            )
            raise TypeError("significant_digits must be an integer or None.")

        eps = numpy.finfo("double").eps
        if significant_digits is None or significant_digits > numpy.abs(
            numpy.log10(eps)
        ):
        ...
        Xrank = numpy.sum(
            numpy.abs(numpy.diag(R1))
            > 10 ** (numpy.log10(numpy.abs(R1[0, 0])) - significant_digits) * max(n, p1)
        )
        Yrank = numpy.sum(
            numpy.abs(numpy.diag(R2))
            > 10 ** (numpy.log10(numpy.abs(R2[0, 0])) - significant_digits) * max(n, p2)
        )

        # We validate this here, to avoid computing the rank twice.
        if X.shape[0] < Xrank or X.shape[0] < Yrank:
            cherrypy.log.error(
                "cca.py", "Number of rows must be >= rank(X), and >= rank(Y)."
            )
        ...
        x_loadings = numpy.array(
            [[scipy.stats.pearsonr(i, j)[0] for j in X.T] for i in x.T]
        ).T
        y_loadings = numpy.array(
            [[scipy.stats.pearsonr(i, j)[0] for j in Y.T] for i in y.T]
        ).T
        ...
        nondegenerate = r < 1
        wilks = numpy.exp(
            numpy.cumsum(numpy.log(1 - (r[nondegenerate] ** 2))[::-1])[::-1]
        )
    ...
    context.register_wizard(
        "new-cca", "New CCA Model", require={"action": "create", "context": "project"}
    )
    context.register_wizard(
        "rerun-cca",
        "Modified CCA Model",
        require={"action": "create", "context": "model", "model-type": ["cca"]},
    )
