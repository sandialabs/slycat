    ...
    def authenticate(realm, rules=None):
        ...
        if "slycatauth" in cherrypy.request.cookie:
            ...
            try:
                ...
                cherrypy.log.error(
                    "++ PKI Auth logging exception: %s, couchDB session not found for %s"
                    % (e, remote_ip)
                )
            ...
            if session is None:
                cherrypy.log.error(
                    "++ PKI Auth: redirecting %s to SSO_session" % remote_ip
                )
            ...
            cherrypy.log.error(
                "++ PKI Auth: no cookie found, redirecting %s to SSO_session"
                % remote_ip
            )
            slycat.web.server.create_single_sign_on_session(remote_ip, auth_user)

    context.register_tool(
        "slycat-sso-pki-authentication", "on_start_resource", authenticate
    )
