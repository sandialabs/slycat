...
def register_slycat_plugin(context):
    ...
    def authenticate(realm, rules=None):
        ...
        hdr_val = slycat.web.server.config["slycat-web-server"]["auth-parse"][
            "header-key"
        ]
        delim = slycat.web.server.config["slycat-web-server"]["auth-parse"]["delimeter"]
        username_idx = slycat.web.server.config["slycat-web-server"]["auth-parse"][
            "username-index"
        ]
        ...
        try:
            ...
            raise cherrypy.HTTPError(
                "407 Proxy Authentication Required: Slycat could not parse an authorized username."
            )

        # validate that header auth parsing returns a plausible username
        if len(auth_user) > 0 and isinstance(
            auth_user, str
        ):  # tests for str and unicode
            # if len(auth_user) > 0 and isinstance(auth_user, str):        # python 3 version
            pass
        else:
            raise cherrypy.HTTPError(
                "407 Proxy Authentication Required: Slycat did not receive a valid username string."
            )
        ...
        if "slycatauth" in cherrypy.request.cookie:
            ...
            try:
                with slycat.web.server.database.couchdb.get_session_lock(sid):
                    ...
                    # cherrypy.log.error("%s ::: %s" % (datetime.datetime.now(datetime.timezone.utc) - datetime.datetime.strptime(unicode(started),'%Y-%m-%dT%H:%M:%S.%f%z'),cherrypy.request.app.config["slycat"]["session-timeout"]))
            ...
            if session is None:
                cherrypy.log.error(
                    "++ no session found redirecting %s to SSO_session" % remote_ip
                )
                # Successful authentication, create a session and return.
                slycat.web.server.create_single_sign_on_session(remote_ip, auth_user)
        else:
            cherrypy.log.error(
                "++ no cookie found redirecting %s to SSO_session" % remote_ip
            )
            slycat.web.server.create_single_sign_on_session(remote_ip, auth_user)

    context.register_tool(
        "slycat-single-sign-on-authentication", "on_start_resource", authenticate
    )
