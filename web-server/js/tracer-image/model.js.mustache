...
Model.prototype.metadata_loaded = function() {
  ...
  if(this.metadata && layout.bookmark) {
    grid.plots.forEach(function(plot) {
      ...
      plot.images_index = layout.bookmark["images-selection-" + plot.plot_id] || model.image_columns[0];
...
Model.prototype.get_image_column = function(index, success_callback) {
  $.ajax({
    type : "GET",
    url : "{{server-root}}models/{{_id}}/arraysets/data-table/arrays/0/attributes/" + index + "/chunk?ranges=0," + this.metadata["row-count"],
