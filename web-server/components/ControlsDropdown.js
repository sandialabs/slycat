  ...
  render() {
    let optionItems = this.props.items.map((item) => (
      <button
        ...
        onClick={(e) =>
          this.props.set_selected(item.key, this.props.state_label, this.props.trigger, e)
        }
        style={item.style}
      >
        {item.name}
    ...
    let dropdown = (
      <React.Fragment>
        ...
        <div className="dropdown-menu" aria-labelledby={this.props.id} ref={this.dropdown_menu}>
          {optionItems}
        </div>
      </React.Fragment>
    );
