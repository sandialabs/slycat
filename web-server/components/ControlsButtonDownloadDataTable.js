...
class ControlsButtonDownloadDataTable extends React.Component {
  ...
  handleClick() {
    if (
      (this.props.selection === undefined || this.props.selection.length === 0) &&
  ...
  _write_data_table(selectionList) {
    ...
    $.ajax({
      ...
      success: function (result) {
        ControlsButtonDownloadDataTable._write_csv(
  ...
  openCSVSaveChoiceDialog() {
    ...
    let buttons_save = [
      ...
      { className: "btn-primary", label: "Save Entire Table", icon_class: "fa-solid fa-table" },
    ];
    ...
    if (this.props.hidden_simulations !== undefined && this.props.hidden_simulations.length > 0) {
      let visibleRows = this.props.metadata["row-count"] - this.props.hidden_simulations.length;
    ...
    dialog({
      ...
      callback: function (button) {
        ...
        else if (button?.label === "Save Selected") self._write_data_table(self.props.selection);
        else if (button?.label === "Save Visible") self._write_data_table(self._filterIndices());
      },
    });
  }
