...
class ModelsList extends React.Component {
  ...
  componentDidMount() {
    ...
    // setTimeout( () => {
    this._asyncRequest = client.get_configuration_markings_fetch().then((markings) => {
    ...
    return (
      ...
      >
        ...
        {this.props.result == "failed" && (
          <span className="badge badge-danger" title={this.props.message}>
            Failed
          </span>
        )}
    ...
    if (templates.length > 0) {
      return (
        <div className="container">
          ...
          <div className="card">
            <div className="list-group list-group-flush">
              <React.Fragment>{templates}</React.Fragment>
            </div>
          </div>
        </div>
      );
  ...
  delete_template(e) {
    ...
    dialog.dialog({
      ...
      message:
        'The template "' +
        this.props.name +
        '" will be deleted immediately and there is no undo.  This will not affect any existing models.',
      buttons: [
...
function renderTemplates(project_id) {
  ...
  client.get_project_references({
    ...
    success: function (result) {
      ...
      const slycat_templates_root = createRoot(document.getElementById("slycat-templates"));
      slycat_templates_root.render(templates_list);
    },
  });
}
