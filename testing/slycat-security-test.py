...
import slycat.web.client.testing
import StringIO
import struct
import sys
import time
...
server_administrator = slycat.web.client.connection(auth=(options.server_administrator, getpass.getpass("%s password: " % options.server_administrator)), host=options.host, proxies={"http":options.http_proxy,"https":options.https_proxy}, verify=not options.no_verify)
project_administrator = slycat.web.client.connection(auth=(options.project_administrator, getpass.getpass("%s password: " % options.project_administrator)), host=options.host, proxies={"http":options.http_proxy,"https":options.https_proxy}, verify=not options.no_verify)
project_writer = slycat.web.client.connection(auth=(options.project_writer, getpass.getpass("%s password: " % options.project_writer)), host=options.host, proxies={"http":options.http_proxy,"https":options.https_proxy}, verify=not options.no_verify)
project_reader = slycat.web.client.connection(auth=(options.project_reader, getpass.getpass("%s password: " % options.project_reader)), host=options.host, proxies={"http":options.http_proxy,"https":options.https_proxy}, verify=not options.no_verify)
non_project_member = slycat.web.client.connection(auth=(options.non_project_member, getpass.getpass("%s password: " % options.non_project_member)), host=options.host, proxies={"http":options.http_proxy,"https":options.https_proxy}, verify=not options.no_verify)
non_server_member = slycat.web.client.connection(auth=(options.non_server_member, getpass.getpass("%s password: " % options.non_server_member)), host=options.host, proxies={"http":options.http_proxy,"https":options.https_proxy}, verify=not options.no_verify)

# Ensure all of the supplied users have the correct attributes ...
slycat.web.client.testing.assert_dict(server_administrator.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email"], matches={"server-administrator":True})
slycat.web.client.testing.assert_dict(server_administrator.request("GET", "/users/%s" % (options.project_administrator)), contains=["name", "email"], matches={"server-administrator":False})
slycat.web.client.testing.assert_dict(server_administrator.request("GET", "/users/%s" % (options.project_writer)), contains=["name", "email"], matches={"server-administrator":False})
slycat.web.client.testing.assert_dict(server_administrator.request("GET", "/users/%s" % (options.project_reader)), contains=["name", "email"], matches={"server-administrator":False})
slycat.web.client.testing.assert_dict(server_administrator.request("GET", "/users/%s" % (options.non_project_member)), contains=["name", "email"], matches={"server-administrator":False})
with slycat.web.client.testing.assert_http_status(404):
...
with slycat.web.client.testing.assert_http_status(401):
...
with slycat.web.client.testing.assert_http_status(403):
...
with slycat.web.client.testing.assert_http_status(401):
  slycat.web.client.testing.assert_dict(non_server_member.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email"], excludes=["server-administrator"])
slycat.web.client.testing.assert_dict(non_project_member.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email"], excludes=["server-administrator"])
slycat.web.client.testing.assert_dict(project_reader.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email"], excludes=["server-administrator"])
slycat.web.client.testing.assert_dict(project_writer.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email"], excludes=["server-administrator"])
slycat.web.client.testing.assert_dict(project_administrator.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email"], excludes=["server-administrator"])
slycat.web.client.testing.assert_dict(server_administrator.request("GET", "/users/%s" % (options.server_administrator)), contains=["name", "email", "server-administrator"])
...
with slycat.web.client.testing.assert_http_status(500):
