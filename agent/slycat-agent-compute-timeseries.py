...
parser.add_argument(
    "--hash", default=None, help="Unique identifier for the output folder."
)
parser.add_argument(
...
try:
    ...
    for timeseries_index in range(_numTimeseries):
        with h5py.File(
            os.path.join(directory_full_path, "timeseries-%s.hdf5" % timeseries_index),
            "r",
        ) as file:
    ...
    with h5py.File(inputs_path, "r") as file:
        ...
        """
        arrayset_inputs = dict(
            aid="inputs", array=0, dimensions=dimensions, attributes=attributes
        )
        with open(
            os.path.join(dirname, "arrayset_inputs.pickle"), "wb"
        ) as arrayset_inputs_pickle:
        ...
        with open(
            os.path.join(dirname, "inputs_attributes_data.pickle"), "wb"
        ) as attributes_file:
    ...
    file_clusters = dict(
        aid="clusters",
        file=json.dumps(sorted(clusters.keys())),
    ...
    time_ranges = pool.map_sync(
        get_time_range,
        list(itertools.repeat(directory_full_path, timeseries_count)),
    ...
    for index, (name, storage) in enumerate(sorted(clusters.items())):
        ...
        with open(
            os.path.join(dirname, "file_cluster_%s.json" % name), "w"
        ) as file_cluster_n_json:
            json.dump(file_cluster_n, file_cluster_n_json)
        with open(
            os.path.join(dirname, "file_cluster_%s.out" % name), "w"
        ) as file_cluster_n_out:
            json.dump(
                {
        ...
        with open(
            os.path.join(dirname, "waveform_%s_dimensions.pickle" % name), "wb"
        ) as dimensions_file:
            pickle.dump(dimensions_array, dimensions_file)
        with open(
            os.path.join(dirname, "waveform_%s_attributes.pickle" % name), "wb"
        ) as attributes_file:
            pickle.dump(attributes_array, attributes_file)
        with open(
            os.path.join(dirname, "waveform_%s_times.pickle" % name), "wb"
        ) as times_file:
            pickle.dump(waveform_times_array, times_file)
        with open(
            os.path.join(dirname, "waveform_%s_values.pickle" % name), "wb"
        ) as values_file:
