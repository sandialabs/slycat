...
# TODO this function needs to be migrated to the implementation of the computation interface
def generate_batch(module_name, wckey, nnodes, partition, ntasks_per_node, time_hours, time_minutes, time_seconds, fn,
                   tmp_file):
    f = tmp_file
...
def run_function(command):
    ...
    generate_batch(module_name, wckey, nnodes, partition, ntasks_per_node, time_hours, time_minutes, time_seconds, fn,
                   tmp_file)
    results["temp_file"] = commands.getoutput("%s %s" % ("cat", tmp_file.name))
...
def get_file(command):
    ...
    sys.stdout.write("%s\n%s" % (json.dumps(
        {"ok": True, "message": "File retrieved.", "path": path, "content-type": content_type, "size": len(content)}),
                                 content))
    sys.stdout.flush()
