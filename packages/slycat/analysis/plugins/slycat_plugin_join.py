...
def register_client_plugin(context):
  ...
  def join(connection, array1, array2):
    ...
    """
    array1 = slycat.analysis.plugin.client.require_array(array1)
    array2 = slycat.analysis.plugin.client.require_array(array2)
    ...
    if dimensions1 != dimensions2:
      raise slycat.analysis.plugin.client.InvalidArgument("Arrays to be joined must have identical dimensions.")
    return connection.remote_array(connection.coordinator.call_plugin_function("join", connection.require_object(array1), connection.require_object(array2)))
...
def register_coordinator_plugin(context):
  ...
  def join(factory, array1, array2):
    ...
    return factory.pyro_register(slycat.analysis.plugin.coordinator.array(array_workers, [array1, array2]))
  context.register_plugin_function("join", join)
...
def register_worker_plugin(context):
  ...
  class join_array(slycat.analysis.plugin.worker.array):
  ...
  class join_array_iterator(slycat.analysis.plugin.worker.array_iterator):
