    ...
    try:
        ...
        for index, host_session in enumerate(session["sessions"]):
            if host_session["hostname"] == hostname:
                ...
                if host_session[
                    "session_type"
                ] == "ssh" and not slycat.web.server.remote.check_session(sid):
                    cherrypy.log.error(
                        "error %s SID:%s Keys %s"
                        % (
                            slycat.web.server.remote.check_session(sid),
                            sid,
                            list(slycat.web.server.remote.session_cache.keys()),
                        )
                    )
                    ...
                    "session_type"
                ] == "smb" and not slycat.web.server.smb.check_session(sid):
                    cherrypy.log.error(
                        "error %s SID:%s Keys %s"
                        % (
                            slycat.web.server.smb.check_session(sid),
                            sid,
                            list(slycat.web.server.smb.session_cache.keys()),
                        )
                    )
...
def require_json_parameter(name):
    ...
    if name not in cherrypy.request.json:
        cherrypy.log.error(
            "slycat.web.server.handlers.py require_json_parameter",
...
def require_boolean_json_parameter(name):
    ...
    if value is not True and value is not False:
        cherrypy.log.error(
            "slycat.web.server.handlers.py require_boolean_json_parameter",
...
def require_array_json_parameter(name):
    ...
    if not isinstance(array, list):
        cherrypy.log.error(
            "slycat.web.server.handlers.py require_array_json_parameter",
...
def require_integer_array_json_parameter(name):
    ...
    try:
        ...
        cherrypy.log.error(
            "slycat.web.server.handlers.py require_integer_array_json_parameter",
            "cherrypy.HTTPError 400 '%s' parameter must be an array of integers."
            % name,
        )
        raise cherrypy.HTTPError(
            "400 '%s' parameter must be an array of integers." % name
        )
    return array
...
def require_integer_parameter(value, name):
    try:
        ...
        cherrypy.log.error(
            "slycat.web.server.handlers.py require_integer_parameter",
...
def get_projects_list(_=None):
    ...
    projects = [
        project
        for project in database.scan("slycat/projects")
        if slycat.web.server.authentication.is_project_reader(project)
        or slycat.web.server.authentication.is_project_writer(project)
...
def post_projects():
    ...
    cherrypy.response.headers["location"] = "%s/projects/%s" % (
...
def get_remote_host_dict():
    ...
    for host in remote_host_dict:
        if "message" in remote_host_dict[host]:
            remote_host_list.append(
                {"name": host, "message": remote_host_dict[host]["message"]}
            )
        else:
...
def put_project(pid):
    ...
    if "acl" in cherrypy.request.json:
        ...
        if "administrators" not in cherrypy.request.json["acl"]:
            cherrypy.log.error(
                "slycat.web.server.handlers.py put_project",
...
def delete_project(pid):
    ...
    for reference in couchdb.scan(
        "slycat/project-references", startkey=pid, endkey=pid
    ):
...
def put_project_csv_data(pid, file_key, parser, mid, aids):
    ...
    if not project_datas:
        ...
        raise cherrypy.HTTPError(
            "404 There is no project data stored for this project. %s" % file_key
        )
    else:
                ...
                # CSV file path
                    ...
                    with h5py.File(hdf5_path, "r") as hdf5_file:
                        ...
                        cherrypy.log.error(
                            ...
                            % (
                                item,
                                str([[key, val] for key, val in hdf5_file.items()]),
                            )
                        )
...
def get_project_references(pid):
    ...
    references = [
        reference
        for reference in database.scan(
            "slycat/project-references", startkey=pid, endkey=pid
        )
    ]
...
def post_project_models(pid):
    ...
    if model_type not in allowed_model_types:
        cherrypy.log.error(
            "slycat.web.server.handlers.py post_project_models",
            "cherrypy.HTTPError allowed model types: %s"
            % ", ".join(allowed_model_types),
        )
        raise cherrypy.HTTPError(
            "400 Allowed model types: %s" % ", ".join(allowed_model_types)
        )

    marking = require_json_parameter("marking")
    if (
        marking
        not in cherrypy.request.app.config["slycat-web-server"]["allowed-markings"]
    ):
        cherrypy.log.error(
            "slycat.web.server.handlers.py post_project_models",
            "cherrypy.HTTPError 400 allowed marking types: %s"
            % ", ".join(
                cherrypy.request.app.config["slycat-web-server"]["allowed-markings"]
            ),
        )
        raise cherrypy.HTTPError(
            "400 Allowed marking types: %s"
            % ", ".join(
                cherrypy.request.app.config["slycat-web-server"]["allowed-markings"]
            )
        )
    ...
    cherrypy.response.headers["location"] = "%s/models/%s" % (
        cherrypy.request.base,
        mid,
    )
...
def post_project_bookmarks(pid):
    ...
    slycat.web.server.authentication.require_project_reader(
        project
    )  # This is intentionally out-of-the-ordinary - we explicitly allow project *readers* to store bookmarks.

    content = json.dumps(
        cherrypy.request.json, separators=(",", ":"), indent=None, sort_keys=True
    )
    ...
    cherrypy.response.headers["location"] = "%s/bookmarks/%s" % (
        cherrypy.request.base,
        bid,
    )
...
def post_project_references(pid):
    ...
    cherrypy.response.headers["location"] = "%s/references/%s" % (
        cherrypy.request.base,
        rid,
    )
...
def get_project_data_parameter(did, param, **kwargs):
    ...
    try:
        return slycat.web.server.get_project_data_parameter(
            database, project_data, param
        )
    except KeyError as e:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_project_data_parameter",
...
def model_command(mid, type, command, **kwargs):
    ...
    cherrypy.log.error(
        "slycat.web.server.handlers.py model_command",
...
def model_sensitive_command(mid, type, command):
    ...
    cherrypy.log.error(
        "slycat.web.server.handlers.py model_sensitive_command",
        "cherrypy.HTTPError 400 unknown command: %s" % command,
    )
    raise cherrypy.HTTPError("400 Unknown command: %s" % command)
...
def post_model_finish(mid):
    ...
    if model["state"] != "waiting":
        cherrypy.log.error(
            "slycat.web.server.handlers.py post_model_finish",
...
def post_model_files(
    mid, input=None, files=None, sids=None, paths=None, aids=None, parser=None, **kwargs
):
    if input is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py post_model_files",
    ...
    if files is not None and sids is None and paths is None:
        ...
        if len(sids) != len(paths):
            ...
            raise cherrypy.HTTPError(
                "400 sids and paths parameters must have the same length."
            )
        files = []
        ...
        raise cherrypy.HTTPError(
            "400 Must supply files parameter, or sids and paths parameters."
        )
...
def post_uploads():
    ...
    if parser not in slycat.web.server.plugin.manager.parsers:
        cherrypy.log.error(
            "slycat.web.server.handlers.py post_uploads",
            "cherrypy.HTTPError 400 unknown parser plugin: %s." % parser,
        )
        raise cherrypy.HTTPError("400 Unknown parser plugin: %s." % parser)
    ...
    kwargs = {
        key: value
        for key, value in list(cherrypy.request.json.items())
        if key not in ["mid", "input", "parser", "aids"]
    }

    uid = slycat.web.server.upload.create_session(mid, input, parser, aids, kwargs)

    cherrypy.response.headers["location"] = "%s/uploads/%s" % (
        cherrypy.request.base,
        uid,
    )
...
def put_upload_file_part(uid, fid, pid, file=None, hostname=None, path=None):
    ...
    # remote file
        ...
        raise cherrypy.HTTPError(
            "400 Must supply file parameter, or sid and path parameters."
        )
...
def open_id_authenticate(**params):
    ...
    """
    cherrypy.log.error(
        "++ open_id_authenticate starting, incoming params = %s" % params
    )
    # check for openid in the config for security
    if (
        slycat.web.server.config["slycat-web-server"]["authentication"]["plugin"]
        != "slycat-openid-authentication"
    ):
        raise cherrypy.HTTPError(404)
    ...
    cherrypy.log.error(
        "++ open_id_authenticate: setting auth_user = %s, calling create_single_sign_on_session"
        % auth_user
    )
    slycat.web.server.create_single_sign_on_session(
        slycat.web.server.check_https_get_remote_ip(), auth_user
    )
    ...
    try:
        if "slycatauth" in cherrypy.request.cookie:
            ...
            if session is not None:
                cherrypy.response.status = "200 session deleted." + str(
...
def clear_ssh_sessions():
    ...
    try:
        if "slycatauth" in cherrypy.request.cookie:
            ...
            session = couchdb.get("session", sid)
            cherrypy.log.error(
                "ssh and smb sessions cleared for user session: %s" % session
            )
...
def put_model_inputs(mid):
    ...
    if source["project"] != model["project"]:
        cherrypy.log.error(
            "slycat.web.server.handlers.py put_model_inputs",
    ...
    with slycat.web.server.database.couchdb.db_lock:
        try:
            slycat.web.server.put_project_data_parameter(
                database, project_data, aid, value, require_input
            )
        except Exception:
            time.sleep(1)
            database = slycat.web.server.database.couchdb.connect()
    ...
    with slycat.web.server.database.couchdb.db_lock:
        try:
            slycat.web.server.put_model_parameter(
                database, model, aid, value, require_input
            )
        except Exception:
            time.sleep(1)
            database = slycat.web.server.database.couchdb.connect()
...
def put_model_arrayset_array(mid, aid, array):
    ...
    slycat.web.server.put_model_array(
        database, model, aid, array_index, attributes, dimensions
    )
...
def put_model_arrayset_data(mid, aid, hyperchunks, data, byteorder=None):
    ...
    try:
        ...
        cherrypy.log.error(
            "slycat.web.server.handlers.py put_model_arrayset_data",
    ...
    if byteorder is not None and byteorder not in ["big", "little"]:
        ...
        raise cherrypy.HTTPError(
            "400 optional byteorder argument must be big or little."
        )
            ...
            for array in slycat.hyperchunks.arrays(
                ...
                for attribute in array.attributes(len(hdf5_array.attributes)):
                    if not isinstance(
                        ...
                        raise cherrypy.HTTPError(
                            "400 Cannot assign data to computed attributes."
                        )
                    for hyperslice in attribute.hyperslices():
                        ...
                        if hyperslice == (Ellipsis,):
                            ...
                            data_shape = []
                            for hyperslice_dimension, array_dimension in zip(
                                hyperslice, hdf5_array.dimensions
                            ):
                                if isinstance(hyperslice_dimension, numbers.Integral):
                                    ...
                                    cherrypy.log.error(
                                        "slycat.web.server.handlers.py put_model_arrayset_data",
                                        "Unexpected hyperslice: %s"
                                        % hyperslice_dimension,
                                    )
                                    raise ValueError(
                                        "Unexpected hyperslice: %s"
                                        % hyperslice_dimension
                        ...
                        data_type = slycat.hdf5.dtype(
                            hdf5_array.attributes[attribute.expression.index]["type"]
                        )
                        data_size = numpy.prod(data_shape)

                        if byteorder is None:
                            hyperslice_data = numpy.array(
                                next(data_iterator), dtype=data_type
                            ).reshape(data_shape)
                        elif byteorder == sys.byteorder:
                            hyperslice_data = numpy.fromfile(
                                data.file, dtype=data_type, count=data_size
                            ).reshape(data_shape)
                        else:
                            cherrypy.log.error(
                                "slycat.web.server.handlers.py put_model_arrayset_data",
                        ...
                        hdf5_array.set_data(
                            attribute.expression.index, hyperslice, hyperslice_data
                        )
...
def get_project_cache_object(pid, key):
    ...
    for cache_object in database.scan(
        ...
        cherrypy.response.headers["content-type"] = cache_object["_attachments"][
            "content"
        ]["content_type"]
...
def delete_project_cache_object(pid, key):
    ...
    cherrypy.log.error(
        "slycat.web.server.handlers.py delete_project_cache_object",
...
def get_model_array_attribute_chunk(mid, aid, array, attribute, **arguments):
    try:
        ...
        raise cherrypy.HTTPError(
            "400 Malformed attribute argument must be a zero-based integer attribute index."
        )
    ...
    if byteorder is not None:
        if byteorder not in ["little", "big"]:
            raise cherrypy.HTTPError(
                "400 Malformed byteorder argument must be 'little' or 'big'."
            )
    ...
    with slycat.web.server.hdf5.lock:
        with slycat.web.server.hdf5.open(artifact) as file:
            ...
            if len(ranges) != hdf5_array.ndim:
                raise cherrypy.HTTPError(
                    "400 Ranges argument doesn't contain the correct number of dimensions."
                )

            ranges = [
                (max(dimension["begin"], range[0]), min(dimension["end"], range[1]))
                for dimension, range in zip(hdf5_array.dimensions, ranges)
            ]
...
def get_model_arrayset_metadata(mid, aid, **kwargs):
    ...
    """
    cherrypy.log.error(
        "GET arrayset metadata mid:%s aid:%s kwargs:%s"
        % (mid, aid, list(kwargs.keys()))
    )
    database = slycat.web.server.database.couchdb.connect()
    model = database.get("model", mid)
    project = database.get("project", model["project"])
    ...
    if artifact is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_arrayset_metadata",
    ...
    try:
        arrays = (
            slycat.hyperchunks.parse(kwargs["arrays"]) if "arrays" in kwargs else None
        )
    except:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_arrayset_metadata",
    ...
    try:
        statistics = (
            slycat.hyperchunks.parse(kwargs["statistics"])
    ...
    try:
        unique = (
            slycat.hyperchunks.parse(kwargs["unique"]) if "unique" in kwargs else None
        )
    except:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_arrayset_metadata",
        ...
        raise cherrypy.HTTPError("400 Not a valid hyperchunks specification.")
    cherrypy.log.error(
        "GET arrayset metadata arrays:%s stats:%s unique:%s"
        % (arrays, statistics, unique)
    )
    results = slycat.web.server.get_model_arrayset_metadata(
        database, model, aid, arrays, statistics, unique
    )
    ...
    if "unique" in results:
        for unique in results["unique"]:
            ...
            if isinstance(results["unique"][0]["values"][0], list):
                unique["values"] = [
                    list_of_values for list_of_values in unique["values"]
                ]
...
def get_model_arrayset_data(mid, aid, hyperchunks, byteorder=None):
    ...
    """
    cherrypy.log.error(
        "GET Model Arrayset Data: arrayset %s hyperchunks %s byteorder %s"
        % (aid, hyperchunks, byteorder)
    )
    try:
        hyperchunks = slycat.hyperchunks.parse(hyperchunks)
    except Exception as e:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_arrayset_data",
            "cherrypy.HTTPError 400 not a valid hyperchunks specification.",
        )
        raise cherrypy.HTTPError(
            "400 Not a valid hyperchunks specification.%s%s"
    ...
    def content():
        if byteorder is None:
            yield json.dumps(
                [
                    mask_nans(hyperslice).tolist()
                    for hyperslice in slycat.web.server.get_model_arrayset_data(
...
def post_model_arrayset_data(mid, aid):
    ...
    try:
        ...
        raise cherrypy.HTTPError("400 Not a valid hyperchunks specification.")
    cherrypy.log.error(
        "GET Model Arrayset Data: arrayset %s hyperchunks calculated" % (aid)
    )
...
def validate_table_rows(rows):
    try:
        ...
        rows = [
            (int(spec[0]), int(spec[1]) if len(spec) == 2 else int(spec[0]) + 1)
            for spec in rows
        ]
        ...
        cherrypy.log.error(
            "slycat.web.server.handlers.py validate_table_rows",
...
def validate_table_columns(columns):
    try:
        ...
        columns = numpy.concatenate(
            [numpy.arange(begin, end) for begin, end in columns]
        )
        columns = columns[columns >= 0]
    except Exception as e:
        cherrypy.log.error(str(e))
        cherrypy.log.error(
            "slycat.web.server.handlers.py validate_table_columns",
...
def validate_table_sort(sort):
    if sort is not None:
        try:
            ...
            cherrypy.log.error(
                "slycat.web.server.handlers.py validate_table_sort",
        ...
        for column, order in sort:
            ...
            if order not in ["ascending", "descending"]:
                ...
                raise cherrypy.HTTPError(
                    "400 Sort-order must be 'ascending' or 'descending'."
                )
        ...
        if index is not None and sort_column == metadata["column-count"] - 1:
            ...
            if index_key not in file:
                ...
                sort_index = numpy.argsort(
                    slycat.hdf5.ArraySet(file)[array_index].get_data(sort_column)[...],
                    kind="mergesort",
                )
...
def get_table_metadata(file, array_index, index):
    ...
    if array.ndim != 1:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_table_metadata",
    ...
    statistics = [
        array.get_statistics(attribute) for attribute in range(len(attributes))
    ]
...
def get_model_table_metadata(mid, aid, array, index=None):
    ...
    if artifact is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_table_metadata",
            "cherrypy.HTTPError 404 artifact is None for aid: %s" % aid,
        )
        raise cherrypy.HTTPError(404)
    artifact_type = model["artifact-types"][aid]
    ...
    with slycat.web.server.hdf5.lock:
        with slycat.web.server.hdf5.open(
            artifact, "r+"
        ) as file:  # We have to open the file with writing enabled because the statistics cache may need to be updated.
...
@cherrypy.tools.json_out(on=True)
def get_model_table_chunk(
    mid, aid, array, rows=None, columns=None, index=None, sort=None
):
    ...
    if artifact is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_table_chunk",
            "cherrypy.HTTPError 404 artifact is None for aid: %s" % aid,
        )
        raise cherrypy.HTTPError(404)
    artifact_type = model["artifact-types"][aid]
        ...
        with slycat.web.server.hdf5.open(artifact, mode="r+") as file:
            ...
            for column in columns:
                ...
                if index is not None and column == metadata["column-count"] - 1:
                    ...
                    values = (
                        slycat.hdf5.ArraySet(file)[array]
                        .get_data(column)[sort_slice[slice_index].tolist()][
                            slice_reverse_index
                        ]
                        .tolist()
                    )
                    if meta_type in ["float32", "float64"]:
                        values = [
                            None if numpy.isnan(value) else value for value in values
                        ]
                data.append(values)
            ...
            result = {
                ...
                "column-names": [
                    metadata["column-names"][column] for column in columns
                ],
                "data": data,
                "sort": sort,
            }
...
def get_model_table_sorted_indices(
    mid, aid, array, rows=None, index=None, sort=None, byteorder=None
):
    rows = validate_table_rows(rows)
    sort = validate_table_sort(sort)
    ...
    if artifact is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_table_sorted_indices",
            "cherrypy.HTTPError 404 artifact is None for aid: %s" % aid,
        )
        raise cherrypy.HTTPError(404)
    artifact_type = model["artifact-types"][aid]
    ...
    with slycat.web.server.hdf5.lock:
        with slycat.web.server.hdf5.open(artifact, mode="r+") as file:
            ...
            # Retrieve the data ...
            sort_index = get_table_sort_index(file, metadata, array, sort, index)
            slice_val = numpy.argsort(sort_index, kind="mergesort")[rows].astype(
                "int32"
            )
...
def get_model_table_unsorted_indices(
    ...
    if artifact is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_table_unsorted_indices",
            "cherrypy.HTTPError 404 artifact is None for aid: %s" % aid,
        )
        raise cherrypy.HTTPError(404)
    artifact_type = model["artifact-types"][aid]
...
def get_model_file(mid, aid):
    ...
    if artifact is None:
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_file",
            "cherrypy.HTTPError 404 artifact is None for aid: %s" % aid,
        )
        raise cherrypy.HTTPError(404)
    artifact_type = model["artifact-types"][aid]
    ...
    cherrypy.response.headers["content-type"] = model["_attachments"][fid][
        "content_type"
    ]
...
def get_model_parameter(mid, aid):
    ...
    try:
        ...
        cherrypy.log.error(
            "slycat.web.server.handlers.py get_model_parameter",
...
def get_model_statistics(mid):
    ...
    # get hdf5 root dir
    hdf5_root_directory = cherrypy.tree.apps[""].config["slycat-web-server"][
        "data-store"
    ]
    ...
    for key, value in model["artifact-types"].items():
        if value == "hdf5":
            ...
            hdf5_file_path = os.path.join(
                hdf5_root_directory, array[0:2], array[2:4], array[4:6], array + ".hdf5"
            )
    ...
    try:
        server_cache_size = (
            float(sys.getsizeof(_pickle.dumps(slycat.web.server.server_cache.cache)))
            / 1024.0
            / 1024.0
        )
    except:
    ...
    return {
        ...
        "hdf5_footprint": 100.0
        * (float(hdf5_file_size) / float(total_hdf5_server_size)),
        ...
        "analysis_computation_time": (
            0.0
            if "analysis_computation_time" not in model
            else float(model["analysis_computation_time"])
        ),
        "db_creation_time": (
            0.0 if "db_creation_time" not in model else float(model["db_creation_time"])
        ),
    }
    ...
    try:
        ...
        for i in range(len(session["sessions"])):
            if session["sessions"][i]["hostname"] == hostname:
                if (
                    "sid" in session["sessions"][i]
                    and session["sessions"][i]["sid"] is not None
                ):
                    slycat.web.server.remote.delete_session(
                        session["sessions"][i]["sid"]
                    )
...
def post_combine_hdf5_tables(mid):
    ...
    dimensions = [
        {"name": "row", "type": "int64", "begin": 0, "end": len(combined_dataset[0])}
    ]
    ...
    with slycat.web.server.database.couchdb.db_lock:
        ...
        slycat.web.server.put_model_array(
            database, model, "data-table", 0, attributes, dimensions
        )
        slycat.web.server.put_model_arrayset_data(
            database, model, "data-table", "%s/.../..." % array_index, combined_data
        )
    ...
    try:
        ...
        for h_session in session["sessions"]:
            if h_session["hostname"] == hostname:
                if h_session[
                    "session_type"
                ] == "ssh" and slycat.web.server.remote.check_session(h_session["sid"]):
                    status = True
                    msg = "hostname session was found"
                elif h_session[
                    "session_type"
                ] == "smb" and slycat.web.server.smb.check_session(h_session["sid"]):
                    status = True
                    msg = "hostname session was found"
                    ...
                    session["sessions"][:] = [
                        tup
                        for tup in session["sessions"]
                        if tup["hostname"] != hostname
                    ]
                    database.save(session)
...
def get_remote_show_user_password():
    ...
    msg = "unknown"
    if (
        cherrypy.request.app.config["slycat-web-server"]["remote-authentication"][
            "method"
        ]
        == "password"
    ):
    ...
    if (
        cherrypy.request.app.config["slycat-web-server"]["authentication"]["plugin"]
        == "slycat-password-authentication"
    ):
        slycat_pass = True
...
def post_remote_browse(hostname, path):
    sid, session_type = get_sid(hostname)
    file_reject = (
        re.compile(cherrypy.request.json.get("file-reject"))
        if "file-reject" in cherrypy.request.json
        else None
    )
    file_allow = (
        re.compile(cherrypy.request.json.get("file-allow"))
        if "file-allow" in cherrypy.request.json
        else None
    )
    directory_reject = (
        re.compile(cherrypy.request.json.get("directory-reject"))
        if "directory-reject" in cherrypy.request.json
        else None
    )
    directory_allow = (
        re.compile(cherrypy.request.json.get("directory-allow"))
    ...
    with slycat.web.server.remote.get_session(sid) as session:
        return session.browse(
            path, file_reject, file_allow, directory_reject, directory_allow
        )
...
def get_time_series_names(hostname, path, **kwargs):
    ...
    if len(response_time_series_names) >= 1:
        ...
        raise cherrypy.HTTPError(
            "400 could not detect timeseries names. There could be hidden characters in your csv"
        )
...
def get_configuration_markings():
    return [
        dict(list(marking.items()) + [("type", key)])
        for key, marking in list(slycat.web.server.plugin.manager.markings.items())
        if key in cherrypy.request.app.config["slycat-web-server"]["allowed-markings"]
    ]
...
def get_selectable_configuration_markings():
    return [
        ...
        for key, marking in list(slycat.web.server.plugin.manager.markings.items())
        if key
        in cherrypy.request.app.config["slycat-web-server"]["selectable-markings"]
    ]
...
def get_configuration_parsers():
    return [
        {"type": key, "label": parser["label"], "categories": parser["categories"]}
        for key, parser in list(slycat.web.server.plugin.manager.parsers.items())
    ]
...
def get_configuration_version():
    ...
    return {
        ...
        "commit": get_configuration_version.commit.decode("utf-8"),
    }
...
def get_configuration_wizards():
    return [
        dict([("type", type)] + list(wizard.items()))
        for type, wizard in list(slycat.web.server.plugin.manager.wizards.items())
    ]
