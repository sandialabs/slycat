...
def start(root_path, config_file):
  ...
  dispatcher.connect("post-agent-file", "/agents/file{path:.*}", slycat.web.server.handlers.post_agent_file, conditions={"method" : ["POST"]})
  dispatcher.connect("post-agent-image", "/agents/image{path:.*}", slycat.web.server.handlers.post_agent_image, conditions={"method" : ["POST"]})
