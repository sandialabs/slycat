...
def start(root_path, config_file):
  ...
  for key in configuration["slycat"].keys():
    if key not in ["access-log", "access-log-count", "access-log-size", "allowed-markings", "couchdb-database", "couchdb-host", "daemon", "data-store", "directory", "error-log", "error-log-count", "error-log-size", "feed-server-port", "gid", "password-check", "pidfile", "plugins", "projects-redirect", "remote-hosts", "root-path", "server-admins", "server-root", "stdout-log", "stderr-log", "ssl-ciphers", "support-email", "uid", "umask"]:
  ...
  dispatcher.connect("get-ticket", "/tickets", slycat.web.server.handlers.get_ticket, conditions={"method" : ["GET"]})
