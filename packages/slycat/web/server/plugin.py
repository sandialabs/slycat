...
import cherrypy
import hashlib
from slycat.web.server import find_and_load_module
import os
import traceback
...
class Manager(object):
    ...
    def _load_directory(self, plugin_directory):
        try:
            cherrypy.log.error(
                "Loading plugin modules from directory '%s'" % plugin_directory
            )
            plugin_paths = [
                x for x in os.listdir(plugin_directory) if x.endswith(".py")
            ]
    ...
    def _load_module(self, plugin_path):
        try:
            ...
            module = find_and_load_module(
                plugin_name[:-3], plugin_path, plugin_directory
            )
    ...
    def load(self, plugin_path):
        ...
        if not os.path.isabs(plugin_path):
            cherrypy.log.error(
                "slycat.web.server.plugin.py load",
                "Plugin module path must be absolute: %s" % plugin_path,
            )
    ...
    def register_directory(self, type, init, user):
        ...
        if type in self.directories:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_directory",
                "Directory type '%s' has already been registered." % type,
            )
    ...
    def register_marking(self, type, label, badge, page_before=None, page_after=None):
        ...
        if type in self.markings:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_marking",
                "Marking type '%s' has already been registered." % type,
            )
    ...
    def register_page_bundle(self, type, content_type, paths):
        ...
        for path in paths:
            if not os.path.isabs(path):
                cherrypy.log.error(
                    "slycat.web.server.plugin.py register_page_bundle",
                    "Bundle file '%s' must be an absolute path." % (path),
                )
    ...
    def register_utility(self, command, handler):
        ...
        if command in self.utilities:
            raise Exception(
                "Utility command '%s' has already been registered." % command
            )
    ...
    def register_model_command(self, verb, type, command, handler):
        ...
        if verb not in ["GET", "POST", "PUT"]:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_model_command",
                "Not an allowed HTTP verb: %s" % verb,
            )
        ...
        if key in self.model_commands:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_model_command",
                "Command '%s %s %s' has already been registered."
                % (verb, type, command),
            )
            raise Exception(
                "Command '%s %s %s' has already been registered."
        ...
        cherrypy.log.error(
            "Registered custom command '%s %s %s'." % (verb, type, command)
        )
    ...
    def register_page_resource(self, type, resource, path):
        ...
        if type not in self.pages:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_page_resource",
                "Unknown page type: %s." % type,
            )
            raise Exception("Unknown page type: %s." % type)
        ...
        if resource in self.page_resources[type]:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_page_resource",
                "Resource '%s' has already been registered with page '%s'."
                % (resource, type),
            )
            raise Exception(
                "Resource '%s' has already been registered with page '%s'."
        ...
        if not os.path.isabs(path):
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_page_resource",
                "Resource '%s' must have an absolute path." % (resource),
            )
    ...
    def register_model(self, type, finish, ptype=None):
        ...
        if type in self.models:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_model",
                "Model type '%s' has already been registered." % type,
            )
        ...
        if not isinstance(ptype, str):
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_model",
                "Page type '%s' must be a string." % ptype,
            )
    ...
    def register_page(self, type, html):
        ...
        if type in self.pages:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_page",
                "Page type '%s' has already been registered." % type,
            )
    ...
    def register_parser(self, type, label, categories, parse):
        ...
        if type in self.parsers:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_parser",
                "Parser type '%s' has already been regiitered.",
            )
    ...
    def register_password_check(self, type, check):
        ...
        if type in self.password_checks:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_password_check",
                "Password check type '%s' has already been registered.",
            )
            raise Exception(
                "Password check type '%s' has already been registered." % type
            )
    ...
    def register_tool(self, name, hook_point, callable):
        ...
        if name in self.tools:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_tool",
                "Tool '%s' has already been registered.",
            )
    ...
    def register_wizard_resource(self, type, resource, path):
        ...
        if type not in self.wizards:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_wizard_resource",
                "Unknown wizard type: %s." % type,
            )
        ...
        if resource in self.wizard_resources[type]:
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_wizard_resource",
                "Resource '%s' has already been registered with wizard '%s'."
                % (resource, type),
            )
            raise Exception(
                "Resource '%s' has already been registered with wizard '%s'."
        ...
        if not os.path.isabs(path):
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_wizard_resource",
                "Resource '%s' must have an absolute path.",
            )
            raise Exception("Resource '%s' must have an absolute path." % (path))
        if not os.path.exists(path):
            cherrypy.log.error(
                "slycat.web.server.plugin.py register_wizard_resource",
                "Resource '%s' does not exist." % (path),
            )
