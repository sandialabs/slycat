...
def compute(mid):
  try:
    raise Exception("Internal timeseries computation disabled.")
...
#        slycat.web.server.update_model(database, model, progress=progress_begin)
#
#        # Get the minimum and maximum times across every series in the cluster
#        slycat.web.server.update_model(database, model, message="Collecting statistics for %s" % name)
#        min_times = [slycat.hdf5.get_array_attribute(file, array_index, 0).attrs["min"] for array_index, attribute_index in storage]
#        max_times = [slycat.hdf5.get_array_attribute(file, array_index, 0).attrs["max"] for array_index, attribute_index in storage]
...
#        slycat.web.server.update_model(database, model, message="Rebinning data for %s" % name)
#
#        waveforms = []
#
...
#            original_times = slycat.hdf5.get_array_attribute(file, array_index, 0)[...]
#            original_values = slycat.hdf5.get_array_attribute(file, array_index, attribute_index)[...]
...
#          slycat.web.server.update_model(database, model, progress=mix(progress_begin, progress_end, float(i) / float(observation_count)))
#          slycat.web.server.update_model(database, model, message="Computing distance matrix for %s, %s of %s" % (name, i+1, observation_count))
...
#        slycat.web.server.update_model(database, model, message="Clustering %s" % name)
...
#          slycat.web.server.update_model(database, model, message="Identifying examplars for %s, %s of %s" % (name, i+1, len(linkage)))
